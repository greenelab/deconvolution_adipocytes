...
# }
# 
# all_sc_expr <- cbind(rep1_sc_df,rep2_sc_df,rep3_sc_df,rep4_sc_df,rep5_sc_df,rep6_sc_df,rep7_sc_df,rep8_sc_df)
# 
# colnames(all_sc_expr) <- all_sc_overlap_cells_labeled[,1]
#   #using cell barcodes for column names
# rownames(all_sc_expr) <- rep1_sc_features[,1]
#   #using EnsemblID for gene names for greater consistency when merging with adipocyte snRNAseq data
...
read_adipose_files <- function(file){
  df <- read.table(file = file.path(reference_data_dir, file),
             header = TRUE)
  return(df)
}
...
for (file in adipose_file_names) {
  assign(paste0("adipose",(which(adipose_file_names == file))),
         read_adipose_files(file))
}

##also want to change gene names to ensemblID
